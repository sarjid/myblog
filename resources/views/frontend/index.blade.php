     ...
     <div class="container">
       ...
       <div class="active-cat-carusel">
        @forelse ($posts as $post)
            ...
            <p class="date">{{ Carbon\Carbon::parse($post->created_at)->diffForHumans() }}</p>
            <h4><a href="{{ route('post.detail',$post->slug) }}">{{ $post->title }}</a></h4>
            </div>
            @empty
            <div class="item single-cat">
                <h4 class="text-danger">No Posts Found</h4>
            </div>
         @endforelse
       </div>
     </div>
               ...
               <div class="row justify-content-center">
                @forelse ($posts as $post)
                    <div class="single-posts col-lg-4 col-sm-4 mb-3">
                       <img class="img-fluid" src=" {{ asset('storage/post/'.$post->image) }}" alt="{{ $post->title }}">
                       ...
                       <div class="detail">
                        ...
                        <p class="footer">
                            ...
                            <ul class="d-flex space-around">
                                <li>
                                    @guest
                                    <i class="fa fa-thumbs-up" aria-hidden="true"><span> {{ $post->likedUsers->count() }}</span></i>
                                    @else
                                    <a onclick="document.getElementById('like-form-{{ $post->id }}').submit();"><i class="fa fa-thumbs-up" aria-hidden="true" style="color:{{auth()->user()->likedPosts()->where('post_id', $post->id)->count() > 0 ? 'red' : ''}}; cursor:pointer; "><span> {{ $post->likedUsers->count() }}</span></i></a>
                                  ...
                                  @endguest
                                    ...
                                    {{-- <a href="javascript:void(0);" onclick=" toastr.info('To add to your favorite list you have to login first.', 'Info', { closeButton: true, progressBar: true, })"><i class="fa fa-heart-o" aria-hidden="true"></i><span> 0</span></a> --}}
                                </li>
